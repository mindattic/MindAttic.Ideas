...
public class LocalFilePackageBlobStoreTests
{
    ...
    public async Task Save_Then_Open_RoundTripsBytes_AndReturnsConventionPath()
    {
        ...
        var blobPath = await store.SaveAsync("Component", "ui.tooltip", 3, bytes);
        Assert.That(blobPath, Is.EqualTo("Component/ui.tooltip/3.idea"));
    ...
    {
        var store = new LocalFilePackageBlobStore(_root);
        Assert.That(await store.OpenAsync("Component/none/1.idea"), Is.Null);
        Assert.That(await store.ExistsAsync("Component/none/1.idea"), Is.False);
    }
