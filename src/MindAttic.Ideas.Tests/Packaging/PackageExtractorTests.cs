...
public class PackageExtractorTests
{
    ...
    public void Extract_WritesBinEntries_AndIsExtractedSeesTheEntryAssembly()
    {
        ...
        using var idea = IdeaTestArchive.Build(new Dictionary<string, string>
        {
            ...
            {
                ManifestVersion = 1, Category = "Widget", Kind = "code", Key = "ui.tooltip", Version = 2,
                DisplayName = "Tooltip", Sdk = 1, EntryType = "MindAttic.Ideas.Widget.Tooltip.V2", AssemblyName = "Tooltip",
            }),
        ...
        var dir = extractor.Extract(reader, "Widget", "ui.tooltip", 2);

        Assert.Multiple(() =>
        {
            Assert.That(extractor.IsExtracted("Widget", "ui.tooltip", 2, "Tooltip"), Is.True);
    ...
    public void ResolveAsset_FindsExtractedFile_AndGuardsTraversal()
    {
        ...
        using var reader = IdeaArchiveReader.Open(idea);
        extractor.Extract(reader, "Widget", "ui.tooltip", 1);

        Assert.Multiple(() =>
        {
            Assert.That(extractor.ResolveAsset("Widget", "ui.tooltip", 1, "css/tip.css"), Is.Not.Null);
            Assert.That(extractor.ResolveAsset("Widget", "ui.tooltip", 1, "css/missing.css"), Is.Null);
            Assert.That(extractor.ResolveAsset("Widget", "ui.tooltip", 1, "../../../secrets.txt"), Is.Null,
    ...
    {
        var extractor = new PackageExtractor(_root);
        var path = extractor.EntryDllPath("Widget", "ui.tooltip", 3, "Tooltip");
        Assert.That(path, Is.EqualTo(Path.Combine(_root, "Widget", "ui.tooltip", "3", "Tooltip.dll")));
    }
    ...
    {
        var extractor = new PackageExtractor(_root);
        Assert.That(extractor.IsExtracted("Widget", "ui.tooltip", 1, "Tooltip"), Is.False);
    }
}
