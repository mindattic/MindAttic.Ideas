...
public class ContentLifecycleServiceTests
{
    ...
    {
        var pages = new[] { Pub("home", "{{MindAttic.Ideas.Widget.Tooltip.V11}}") };
        var blocking = ContentLifecycleService.FindBlockingPages(
            ContentKind.Widget, "tooltip", 11, pages, otherEnabledVersions: new[] { 12 });
    ...
    {
        var pages = new[] { Pub("home", "{{MindAttic.Ideas.Widget.Tooltip}}") };  // floats to latest
        // V11 is the last enabled version -> deleting it orphans the float -> blocked.
        Assert.That(ContentLifecycleService.FindBlockingPages(ContentKind.Widget, "tooltip", 11, pages, Array.Empty<int>()),
            Is.EquivalentTo(new[] { "home" }));
        // V12 remains enabled -> the float harmlessly moves to V12 -> deleting V11 is allowed.
        Assert.That(ContentLifecycleService.FindBlockingPages(ContentKind.Widget, "tooltip", 11, pages, new[] { 12 }),
            Is.Empty);
    }
    ...
    public void DisabledOrUnpublishedPage_IsNotABlockingReference()
    {
        var pages = new[]
        {
            new PageRef("draft", "{{MindAttic.Ideas.Widget.Tooltip.V11}}", null, null, Enabled: false, IsPublished: true),
            new PageRef("hidden", "{{MindAttic.Ideas.Widget.Tooltip.V11}}", null, null, Enabled: true, IsPublished: false),
        };
        Assert.That(ContentLifecycleService.FindBlockingPages(ContentKind.Widget, "tooltip", 11, pages, Array.Empty<int>()),
            Is.Empty);
    }
